...
class Node:
    ...
    def UCB(self):
        self.U = C*math.sqrt(math.log(self.parent.visits)/self.visits)
    ...
    def back_propagation(self,v):
        if self.parent:
            self.parent.back_propagation(-v)
        self.visits += 1
        self.Q = self.Q + (v - self.Q)/self.visits
...
class Board:
    ...
    def rollout(self):
        ...
        if who == 0:
            ...
            node.back_propagation(-1)
        elif who == 2 and self.isfull():
            node.back_propagation(0)
        else:
            ...
            for action in actions:
                ...
                v = self.simulation()
                self.delmove()
                node.children[action].back_propagation(v)
        while n != 0:
            self.delmove()
            n -= 1


    def getmoves(self):   # select nodes with positive neighbors
        moves = []
        for i in range(4,Width+4):
            for j in range(4,Height+4):
    ...
    def simulation(self):
        ...
        for move in moves:   # live 3, simulate with probability
        ...
        moves = moves
        x,y = random.choice(moves)   # No specific patterns, randomly simulate
        self.move(x,y)
        v = self.simulation()
        self.delmove()
        return v
    ...
    def getbestmove(self):
        ...
        for _ in range(times):
            self.timecontrol = 0
            self.rollout()
        bestmove = max(self.root.children.items(),key= lambda x:x[1].Q)[0]
        return bestmove
